...
### 問題点
上記の疑似コードでは以下の理由で交差が発生する可能性がある：
1. 分別器・処理装置の設置場所は入力で固定されている
2. 理想的な座標と実際の設置場所のズレにより、左右の順序が逆転する可能性
3. 親ノードより左にある子、右にある子の関係が崩れる

### 改善アプローチ1: 貪欲法による逐次配置
```python
def build_no_cross_graph_greedy(N, M, K, sep_pos, proc_pos):
    """
    交差を回避しながら貪欲法でグラフを構築
    """
    used_seps = set()
    ...
    # 搬入口(0, 5000)から最も近い分別器を選択
    entry_sep = find_nearest_unused_separator(0, 5000, sep_pos, used_seps)
    used_seps.add(entry_sep)
    entry_connection = N + entry_sep  # 分別器への接続番号
    
    # BFSで分別器を配置
    queue = [(entry_sep, 0)]  # (separator_index, depth)
    
    while queue and len(used_seps) < M:
        current_sep, depth = queue.pop(0)
        current_pos = sep_pos[current_sep]
        
        # 左右の子を配置（交差チェックしながら）
        for exit_num in [1, 2]:  # exit1, exit2
            if len(used_procs) < N:
                # 処理装置に接続するか、次の分別器に接続するか判断
                if depth >= 3 or len(used_seps) >= M - (N - len(used_procs)):
                    # 処理装置に接続
                    best_proc = find_best_processor_no_cross(
                        current_pos, proc_pos, used_procs, connections, exit_num
                    )
                    if best_proc != -1:
                        used_procs.add(best_proc)
                        connections.append((current_sep, best_proc, exit_num))
                else:
                    # 次の分別器に接続
                    next_sep = find_best_separator_no_cross(
                        current_pos, sep_pos, used_seps, connections, exit_num
                    )
                    if next_sep != -1:
                        used_seps.add(next_sep)
                        connections.append((current_sep, N + next_sep, exit_num))
                        queue.append((next_sep, depth + 1))
    
    return connections

def find_best_separator_no_cross(from_pos, sep_positions, used, existing_connections, exit_num):
    """
    交差しない分別器を選択
    """
    best_sep = -1
    ...
    for i, pos in enumerate(sep_positions):
        ...
        # 新しいベルトコンベアが既存のものと交差しないかチェック
        new_belt = (from_pos, pos)
        if not check_crossing(new_belt, existing_connections, exit_num):
            # スコア計算（距離など）
            score = distance(from_pos, pos)
            if score < best_score:
                best_score = score
                best_sep = i
...
### 改善アプローチ2: 平面スイープによる配置
```python
def build_planar_graph(N, M, K, sep_pos, proc_pos):
    """
    平面グラフとして確実に交差しない構造を構築
    """
    # x座標でソート
    sorted_seps = sorted(enumerate(sep_pos), key=lambda x: x[1][0])
    sorted_procs = sorted(enumerate(proc_pos), key=lambda x: x[1][0])
    
    # 左から右へスイープしながら接続
    connections = []
    active_nodes = []  # 現在アクティブな（出口が空いている）ノード
    
    # 搬入口を起点とする
    entry_x = 0
    active_nodes.append(('entry', entry_x, 5000))
    
    # x座標順に処理
    for x in range(0, 10001, 100):  # 100刻みでスイープ
        # この範囲の分別器を処理
        seps_in_range = [s for s in sorted_seps if x <= s[1][0] < x + 100]
        
        for sep_idx, sep_pos in seps_in_range:
            # 最も近いアクティブノードから接続
            if active_nodes:
                best_parent = min(active_nodes, 
                                key=lambda n: distance(n[1:], sep_pos))
                # 接続を追加
                connections.append((best_parent, sep_idx))
                # アクティブノードを更新
                active_nodes.remove(best_parent)
                active_nodes.append(('sep', sep_idx, sep_pos, 'exit1'))
                active_nodes.append(('sep', sep_idx, sep_pos, 'exit2'))
...
### 改善アプローチ3: 区画分割法
```python
def build_partition_graph(N, M, K, sep_pos, proc_pos):
    """
    平面を区画に分割して、各区画内で独立にグラフを構築
    """
    # 平面を縦に分割（交差を防ぐ）
    num_partitions = min(4, N)  # N個の処理装置を最大4区画に分割
    partition_width = 10000 / num_partitions
    
    partitions = [[] for _ in range(num_partitions)]
    
    # 各要素を区画に割り当て
    for i, pos in enumerate(sep_pos):
        partition_idx = min(int(pos[0] / partition_width), num_partitions - 1)
        partitions[partition_idx].append(('sep', i, pos))
    
    for i, pos in enumerate(proc_pos):
        partition_idx = min(int(pos[0] / partition_width), num_partitions - 1)
        partitions[partition_idx].append(('proc', i, pos))
    
    # 各区画内で独立にグラフを構築
    all_connections = []
    for p_idx, partition in enumerate(partitions):
        # 区画内でツリー構造を構築
        connections = build_tree_in_partition(partition, p_idx, num_partitions)
