...
struct State {
  ...
  vector<P> get_safe_pickable_boxes() const {
    vector<P> pickable;
    
    // 現在持っている箱の総重量を計算
    int current_total_weight = 0;
    for (const auto& box : hand) {
      current_total_weight += box.w;
    }
    ...
    for (int i = min_x; i <= max_x; i++) {
      for (int j = min_y; j <= max_y; j++) {
        ...
        if (grid[i][j].d > dist_to_home * current_total_weight) {
  ...
  while (current_state.remaining_boxes > 0) {
    ...
    while (current_state.x != 0) {
      ...
      if (current_state.grid[current_state.x][current_state.y].exist) {
        ...
        int total_w = dist_to_home * current_state.grid[current_state.x][current_state.y].w;
        if (min_d - total_w > 0) {
          current_state.pickup();
          min_d -= total_w;
