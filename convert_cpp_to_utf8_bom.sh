...
# Convert only C++ and header files to UTF-8 with BOM

echo "Converting C++ and header files to UTF-8 with BOM..."
...
# Process only C++ and header files in main directories
find /mnt/c/Programming/AHC -type f \( -name "*.cpp" -o -name "*.h" \) \
    -not -path "*/in/*" -not -path "*/out/*" \
    -not -path "*/x64/*" -not -path "*/.vs/*" \
    -not -path "*/Debug/*" -not -path "*/Release/*" | \
