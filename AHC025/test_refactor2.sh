...
for i in {0..99}; do
    ...
    original_score=$(tail -n 1 original_output/${case_num}.txt | grep -oP 'score =\s*\K[0-9]+')
    
    # Run refactored
    ./AHC025_refactored < in/${case_num}.txt > refactored_output/${case_num}.txt 2>&1
    refactored_score=$(tail -n 1 refactored_output/${case_num}.txt | grep -oP 'score =\s*\K[0-9]+')
