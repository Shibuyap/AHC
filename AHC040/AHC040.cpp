...
WidthNode wNodes[MAX_N * 2 + 10];
HeightNode hNodes[MAX_N * 2 + 10];
int CalcScore2(const vector<Column>& columns, bool cheat, int& ww, int& hh) {
  rep(i, n) {
    wNodes[i * 2].isLeft = true;
    wNodes[i * 2].num = i;
    wNodes[i * 2].point = 0;
    wNodes[i * 2 + 1].isLeft = false;
    wNodes[i * 2 + 1].num = i;
    wNodes[i * 2 + 1].point = 0;
    hNodes[i * 2].isUp = true;
    hNodes[i * 2].num = i;
    hNodes[i * 2 + 1].point = 0;
    hNodes[i * 2 + 1].isUp = false;
    hNodes[i * 2 + 1].num = i;
    hNodes[i * 2 + 1].point = 0;
  ...
  // 番兵
  wNodes[n * 2].isLeft = false;
  wNodes[n * 2].num = -1;
  wNodes[n * 2].prev = -1;
  wNodes[n * 2].next = n * 2 + 1;
  wNodes[n * 2].point = 0;
  wNodes[n * 2 + 1].isLeft = true;
  wNodes[n * 2 + 1].num = -1;
  wNodes[n * 2 + 1].prev = n * 2;
  wNodes[n * 2 + 1].next = -1;
  wNodes[n * 2 + 1].point = INT_INF;
  hNodes[n * 2].isUp = false;
  hNodes[n * 2].num = -1;
  hNodes[n * 2].prev = -1;
  hNodes[n * 2].next = n * 2 + 1;
  hNodes[n * 2].point = 0;
  hNodes[n * 2 + 1].isUp = true;
  hNodes[n * 2 + 1].num = -1;
  hNodes[n * 2 + 1].prev = n * 2;
  hNodes[n * 2 + 1].next = -1;
  hNodes[n * 2 + 1].point = INT_INF;
  ...
  for (auto col : columns) {
    ...
    if (col.dir == 0) {
      ...
      // 左右方向の座標値
      {
        ...
        wNodes[num * 2].point = wNodes[now].point;
        wNodes[num * 2 + 1].point = wNodes[num * 2].point + wid;
        // リンクの更新
        while (true) {
          if (wNodes[now].point == wNodes[num * 2].point && wNodes[now].isLeft == false) {
            now = wNodes[now].next;
          }
          else {
            break;
        ...
        wNodes[num * 2].prev = wNodes[now].prev;
        wNodes[num * 2].next = now;
        wNodes[wNodes[num * 2].prev].next = num * 2;
        wNodes[wNodes[num * 2].next].prev = num * 2;
        while (true) {
          if (wNodes[now].point >= wNodes[num * 2 + 1].point) {
            break;
          }
          now = wNodes[now].next;
        }
        wNodes[num * 2 + 1].prev = wNodes[now].prev;
        wNodes[num * 2 + 1].next = now;
        wNodes[wNodes[num * 2 + 1].prev].next = num * 2 + 1;
        wNodes[wNodes[num * 2 + 1].next].prev = num * 2 + 1;
      }

      // 上下方向の座標値
      {
        hNodes[num * 2].point = 0;
        hNodes[num * 2 + 1].point = 0;
        int now = num * 2;
        while (true) {
          int pre = wNodes[now].prev;
          if (wNodes[now].point == wNodes[pre].point && wNodes[pre].isLeft == true) {
            now = pre;
          }
          else {
            break;
        ...
        int right = wNodes[num * 2 + 1].point;
        int nowH = n * 2;
        while (true) {
          now = wNodes[now].next;
          if (wNodes[now].point > right) {
            break;
          }
          if (wNodes[now].point == right && wNodes[now].isLeft == true) {
            break;
          }
          int now2 = now;
          if (now2 % 2 == 0) {
          ...
          if (hNodes[num * 2].point < hNodes[now2].point) {
            hNodes[num * 2].point = hNodes[now2].point;
            nowH = now2;
          }
        }
        hNodes[num * 2 + 1].point = hNodes[num * 2].point + hei;


        // リンクの更新
        now = nowH;
        while (true) {
          if (hNodes[now].point == hNodes[num * 2].point && hNodes[now].isUp == false) {
            now = hNodes[now].next;
          }
          else {
            break;
        ...
        hNodes[num * 2].prev = hNodes[now].prev;
        hNodes[num * 2].next = now;
        hNodes[hNodes[num * 2].prev].next = num * 2;
        hNodes[hNodes[num * 2].next].prev = num * 2;
        while (true) {
          if (hNodes[now].point >= hNodes[num * 2 + 1].point) {
            break;
          }
          now = hNodes[now].next;
        }
        hNodes[num * 2 + 1].prev = hNodes[now].prev;
        hNodes[num * 2 + 1].next = now;
        hNodes[hNodes[num * 2 + 1].prev].next = num * 2 + 1;
        hNodes[hNodes[num * 2 + 1].next].prev = num * 2 + 1;
      }
    }
    else {
      // LEFTの場合
      ...
      // 上下方向の座標値
      {
        ...
        hNodes[num * 2].point = hNodes[now].point;
        hNodes[num * 2 + 1].point = hNodes[num * 2].point + hei;
        // リンクの更新
        while (true) {
          if (hNodes[now].point == hNodes[num * 2].point && hNodes[now].isUp == false) {
      ...
      // 左右方向の座標値
      {
        wNodes[num * 2].point = 0;
        wNodes[num * 2 + 1].point = 0;
        int now = num * 2;
        while (true) {
          int pre = hNodes[now].prev;
          if (hNodes[now].point == hNodes[pre].point && hNodes[pre].isUp == true) {
            now = pre;
          }
          else {
            break;
        ...
        int down = hNodes[num * 2 + 1].point;
        int nowW = n * 2;
        while (true) {
          now = hNodes[now].next;
          if (hNodes[now].point > down) {
            break;
          }
          if (hNodes[now].point == down && hNodes[now].isUp == true) {
            break;
          }
          int now2 = now;
          if (now2 % 2 == 0) {
          ...
          if (wNodes[num * 2].point < wNodes[now2].point) {
            wNodes[num * 2].point = wNodes[now2].point;
            nowW = now2;
          }
        }
        wNodes[num * 2 + 1].point = wNodes[num * 2].point + wid;
  ...
  //  int now = n * 2;
  //  while (now != n * 2 + 1) {
  //    int nxt = wNodes[now].next;
  //    if (wNodes[now].point == wNodes[nxt].point && wNodes[now].isLeft == true && wNodes[nxt].isLeft == false) {
  //      cout << "NG";
  //    }
  //    now = nxt;
  //  }
  ...
  rep(i, n) {
    if (cs_use[i] == 1) {
      maxDown = max(maxDown, hNodes[i * 2 + 1].point);
      maxRight = max(maxRight, wNodes[i * 2 + 1].point);
    }
  }
